...
from bot import get_best_matching_intent, get_response
...
data = {
   "intents": [
        {"tag": "greeting",
         "patterns": ["Hi","Hi there", "How are you", "Is anyone there?","Hey","Hola", "Hello", "Good day","who are you"],
         "responses": ["Hello, Shravani's Chatbot this side, I am here to tell you about Shravani, if you want sample questions type OPTIONS"],
         "context": [""]
        },
        {"tag": "goodbye",
        ...
        {"tag": "noanswer",
         "patterns": [],
         "responses": ["Sorry, can't understand you. Please check spelling or you can connect me via email- shravanimjagtap13@gmail.com"],
         "context": [""]
        },
        {"tag": "options",
         ...
         "responses": ["I can answer your question like Introduce yourself, Education, Skills, Internships, Projects, Certifications, Extra Curricular Activities, Strengths, Weekness, Hobbies, Contact Information, Achievements"],
         "context": [""]
        },
        {"tag": "introduction",
        ...
        "What do you believe are the strengths that make you a well-rounded and effective professional?"
         ...
         "responses": ["Strengths: One of my greatest strengths lies in my genuine passion for data science. I thrive on exploring data, uncovering patterns, and deriving meaningful insights that can drive informed decision-making. This passion fuels my motivation to continuously learn and adapt to the ever-evolving landscape of data science technologies and methodologies. Additionally, my analytical mindset allows me to approach complex problems with a structured and strategic perspective. I take pride in my ability to communicate these intricate insights effectively, bridging the gap between technical details and practical applications.\n Weaknesses: While my passion for data science is a significant strength, it can sometimes lead me to become deeply engrossed in a project, and I might find it challenging to step away. I'm aware that balance is crucial, and I'm actively working on maintaining a healthy separation between work and personal time. Moreover, due to the rapid advancements in data science, there's always more to learn. Occasionally, I can become too eager to explore every emerging technology, which might lead to feeling overwhelmed. To address this, I'm developing a focused learning plan to ensure I gain in-depth expertise in areas that align with my goals and the demands of the industry."],
         "context": [""]
        },
        {"tag": "hobbies",
        ...
        "responses": ["You can reach out to me via the following contact details:\nEmail Id- shravanimjagtap13@gmail.com\nLinkedIn Profile https://www.linkedin.com/in/shravani-jagtap-4845761ba\nGitHub Profile https://github.com/Shravani1383\nBlog: [GeeksforGeeks Articles](https://auth.geeksforgeeks.org/user/shravanimjagtap13/articles)"],
        "context": [""]
       }
...
# Your data and chatbot logic go here (the parts above your main() function)

# Preprocess text and calculate similarity functions
def get_response(intent_tag):
    intent = next((item for item in data["intents"] if item["tag"] == intent_tag), None)
    if intent:
        return random.choice(intent["responses"])
    else:
        return "Sorry, can't understand you. Please check spelling or you can connect me via email- shravanimjagtap13@gmail.com."
...
def get_bot_response():
    ...
    if user_input.lower() == "exit":
        ...
        if best_intent and best_intent["tag"] != "noanswer":
            response = get_response(best_intent["tag"])
        else:
            response = get_response("noanswer")
